...
def filt_raw_centrality(name, perc_to_keep, identifier=None,\
    ...
    if (perc_to_keep == 100) or (perc_to_keep is None):
        ...
        values = np.mean(dist_matrix ** 2, axis=1) ** (1 / 2)

        #Take the perc_to_keep percentile of the density
        percentile = np.percentile(values, 100-perc_to_keep)

        if reverse_order:
            percentile = np.percentile(values, perc_to_keep)
            print("(filt_raw) WARNING: reverse_order = True")
            inds = np.argwhere(values < percentile)[:,0] #<----This gives you the (100-perc_to_keep) LEAST dense points, i.e. outliers etc.
        else:
            inds = np.argwhere(values > percentile)[:,0] #<----This gives you the (100-perc_to_keep) MOST dense points
